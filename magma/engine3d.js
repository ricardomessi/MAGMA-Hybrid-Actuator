...
const chartCanvas = document.getElementById('chartTorqueSpeed');
const ctxChart = chartCanvas ? chartCanvas.getContext('2d') : null;
const chartCanvas2 = document.getElementById('chartPowerEff');
const ctxChart2 = chartCanvas2 ? chartCanvas2.getContext('2d') : null;

/* ================================================================
   THREE.JS SETUP — Photorealistic rendering
   ================================================================ */
const container = document.getElementById('viewport3d').parentElement;
...
const vpCanvas = document.getElementById('viewport3d');
vpCanvas.parentElement.appendChild(renderer.domElement);
...
function updateInfoPanel(comp) {
    const s = SPECS[comp];
    if (!s) return;
    // No detailed info panel in MAGMA — just update tooltip
}
...
function updateUI() {
    ...
    if(el('tlRpm')) el('tlRpm').textContent = Math.round(sim.rpm);
    if(el('tlTorque')) el('tlTorque').textContent = sim.torque.toFixed(1);
    if(el('tlPower')) el('tlPower').textContent = sim.power.toFixed(1);
    if(el('tlEff')) el('tlEff').textContent = Math.round(sim.efficiency);
    if(el('tlMode')) el('tlMode').textContent = sim.mode.toUpperCase();

    // Mode badge
    if(el('modeLabel')) el('modeLabel').textContent = sim.mode.toUpperCase();
    if(el('modeDot')) {
        el('modeDot').className = 'viewport-mode-dot' + (sim.mode==='running'?' running':sim.mode==='holding'?' holding':'');
    }
    if(el('navStatus')) el('navStatus').textContent = sim.mode.toUpperCase();
    
    // Widget Telemetry
    if(el('widgetRealRpm')) el('widgetRealRpm').textContent = Math.round(sim.rpm);
    if(el('widgetRealPower')) el('widgetRealPower').textContent = sim.power.toFixed(1) + ' W';
    if(el('widgetModeDot')) {
        el('widgetModeDot').className = 'mode-dot' + (sim.mode==='running'?' running':sim.mode==='holding'?' holding':'');
    }

    // SMA status
    const tp = Math.min(100,((sim.smaTemp-25)/60)*100);
    if(el('smaTempBar')) el('smaTempBar').style.width = tp+'%';
    if(el('smaTemp')) el('smaTemp').textContent = Math.round(sim.smaTemp)+'°C';
    if(el('smaForceBar')) el('smaForceBar').style.width = Math.min(100,sim.smaForce)+'%';
    if(el('smaForce')) el('smaForce').textContent = Math.round(sim.smaForce)+'%';
    if(el('smaContractBar')) el('smaContractBar').style.width = (sim.smaContraction/6*100)+'%';
    if(el('smaContract')) el('smaContract').textContent = sim.smaContraction.toFixed(1)+' mm';
    if(el('smaPhaseLabel')) el('smaPhaseLabel').textContent =
        sim.smaTemp>78?'AUSTENITE (Hot)':sim.smaTemp>68?'TRANSFORMING':'MARTENSITE (Cool)';
    if(el('smaPhaseBox')) {
        const isHot = sim.smaTemp > 78;
        el('smaPhaseBox').className = 'sma-phase-indicator ' + (isHot ? 'sma-phase-hot' : 'sma-phase-cool');
        el('smaPhaseBox').textContent = isHot ? 'AUSTENITE — Wire Contracted' : 'MARTENSITE — Wire Relaxed';
    }
    if(el('smaTempBar')) el('smaTempBar').style.background = sim.smaTemp > 68 ? 'var(--accent)' : 'var(--info)';
...
function drawChart() {
    ...
    ctxChart.beginPath();
    for(let i=1; i<4; i++) { const y = i*(h/4); ctxChart.moveTo(0,y); ctxChart.lineTo(w,y); }
    ctxChart.stroke();
    ...
    const drawLine = (data, color, maxVal) => {
        ...
        ctxChart.beginPath();
        const step = w / (chartData.maxLen - 1);
        for(let i = 0; i < chartData.maxLen; i++) {
            const x = i * step;
            const normVal = Math.min(1, Math.max(0, data[i] / maxVal));
            const y = h - (normVal * h * 0.9) - 5;
            if(i === 0) ctxChart.moveTo(x, y); else ctxChart.lineTo(x, y);
        }
        ctxChart.stroke();
    };
    drawLine(chartData.rpm, '#2563eb', 160);
    drawLine(chartData.torque, '#dc2626', 6);

    // Second chart (Power & Efficiency)
    if (!ctxChart2 || !chartCanvas2) return;
    const w2 = chartCanvas2.width = chartCanvas2.offsetWidth;
    const h2 = chartCanvas2.height = chartCanvas2.offsetHeight;
    ctxChart2.clearRect(0, 0, w2, h2);
    ctxChart2.strokeStyle = 'rgba(0,0,0,0.06)'; ctxChart2.lineWidth = 1;
    ctxChart2.beginPath();
    for(let i=1;i<4;i++){const y=i*(h2/4);ctxChart2.moveTo(0,y);ctxChart2.lineTo(w2,y);}
    ctxChart2.stroke();
    const drawLine2 = (data, color, maxVal) => {
        ctxChart2.strokeStyle = color; ctxChart2.lineWidth = 2; ctxChart2.beginPath();
        const step = w2 / (chartData.maxLen - 1);
        for(let i=0;i<chartData.maxLen;i++){const x=i*step;const nv=Math.min(1,Math.max(0,data[i]/maxVal));const y=h2-(nv*h2*0.9)-5;if(i===0)ctxChart2.moveTo(x,y);else ctxChart2.lineTo(x,y);}
        ctxChart2.stroke();
    };
    drawLine2(chartData.power, '#16a34a', 100);
}
...
window.setView = function(view) {
    ...
    if (view === 'assembled') {
        ...
        document.getElementById('btnAssembled').classList.add('active');
        resetMaterials();
        ...
        document.getElementById('btnExploded').classList.add('active');
        resetMaterials();
...
window.startSim = function(mode) {
    if (mode === 'run') sim.mode = 'running';
    else if (mode === 'hold') sim.mode = sim.mode === 'running' ? 'transition' : 'holding';
    else { sim.mode = 'standby'; highlightComponent(null); }
};

window.toggleWidget = function() {
    document.getElementById('miniSimWidget').classList.toggle('collapsed');
};

const qs = (id) => document.getElementById(id);
if (qs('widgetSpeedSlider')) {
    qs('widgetSpeedSlider').addEventListener('input', (e) => {
        sim.targetRpm = +e.target.value;
        qs('widgetSpeedVal').textContent = e.target.value + ' RPM';
    });
}
if (qs('widgetTorqueSlider')) {
    qs('widgetTorqueSlider').addEventListener('input', (e) => {
        sim.targetTorque = +e.target.value;
        qs('widgetTorqueVal').textContent = (+e.target.value).toFixed(1) + ' Nm';
    });
}

function setActiveView(id) {} // noop, handled by setView
